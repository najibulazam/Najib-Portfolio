...
export default function Contact() {
  return (
    ...
    >
      <h2 className="text-2xl sm:text-4xl font-bold mb-6 text-cyan-500 text-center">Let's Build Something</h2>
      ...
      <div className="mb-16 relative">
        ...
        <div className="relative flex justify-center">
          <span className="bg-white dark:bg-black px-6 text-base font-semibold text-cyan-500">Open to Opportunities</span>
        </div>
      </div>

      <div className="mb-16 max-w-5xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-6 hover:border-cyan-500/50 hover:shadow-md hover:-translate-y-1 transition-all duration-300 group">
            <div className="flex items-center justify-center w-12 h-12 rounded-lg bg-cyan-500/10 text-cyan-500 mb-4 group-hover:scale-110 transition-transform duration-300">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
        {contactInfo.map((contact, idx) => (
          <article
            key={idx}
            className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg p-5 shadow-sm hover:shadow-md hover:border-cyan-500/50 hover:-translate-y-1 transition-all duration-300 flex flex-col items-center text-center group"
          >
            <div className="text-cyan-500 mb-3 group-hover:scale-110 transition-transform duration-300">{contact.icon}</div>
            <h3 className="text-base font-bold mb-2 text-gray-900 dark:text-white">{contact.label}</h3>
            <p className="text-xs text-gray-600 dark:text-gray-400 mb-3 break-all">{contact.value}</p>
            {contact.link && (
              <a
                href={contact.link}
                target={contact.label === 'Email' ? undefined : '_blank'}
                rel={contact.label === 'Email' ? undefined : 'noopener noreferrer'}
                className="bg-cyan-500 hover:bg-cyan-600 text-white font-semibold text-xs py-2 px-3 rounded transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-2 focus:ring-cyan-400"
