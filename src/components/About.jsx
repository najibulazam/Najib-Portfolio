...
export default function About() {
  return (
    <section
      id="about"
      className="min-h-screen px-6 py-8 max-w-5xl mx-auto font-mono text-black dark:text-white bg-white dark:bg-black transition-colors pt-6"
      aria-label="About Me section with bio and tech stack"
    >
      <h2 className="text-4xl font-bold mb-6 text-cyan-400 text-center">About Me</h2>

      <p className="max-w-3xl mx-auto mb-8 text-lg leading-relaxed text-gray-700 dark:text-gray-300">
      ...
      <h3 className="text-2xl font-semibold mb-6 text-cyan-300 text-center">Tech Stack</h3>

      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 max-w-3xl mx-auto">
        {techStack.map(({ name, icon }) => (
          <div
            key={name}
            className="flex flex-col items-center space-y-2 bg-gray-100 dark:bg-gray-800 rounded-lg p-3 shadow-md hover:shadow-cyan-500 transition"
            aria-label={name}
            title={name}
          >
            <div className="text-3xl">{icon}</div>
            <span className="text-xs text-gray-600 dark:text-gray-200 text-center">{name}</span>
          </div>
        ))}
      </div>
    </section>
